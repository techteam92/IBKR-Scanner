...
from config import RTHConfig


class RTHScanner:
    """Scanner for regular trading hours volume analysis"""
    
    def __init__(self, config: RTHConfig, ibkr_client: IBKRClient):
        """
        Initialize RTH scanner
        
        Args:
            config: RTH scanner configuration
    ...
    def scan_ticker(
        ...
        """
        Scan a single ticker for RTH volume
        ...
        try:
            ...
            # Scan each timeframe
            for tf in timeframes:
                today_vol, avg_10d_vol = self.calculator.calculate_rth_volume(
                    df, tf, target_date
                )
        ...
        if not df.empty:
            ...
            if valid_mask.any():
                ...
                if len(filtered_valid) < len(valid_df):
                    print(f"RTH Scanner: {len(valid_df)} valid results, {len(filtered_valid)} passed filters")
